...
export default function SavedSearchesPanel() {
  ...
  const {
    searches, saving, saveSuccess, saveError, setSaveError,
  ...
  const handleSaveCurrentSearch = async () => {
    ...
    const ok = await saveSearch(newName, queryData);
    if (ok) {
      setNewName("");
  ...
  const hasCurrentSearch =
    ...
    (Array.isArray(logicalQuery?.rules) && logicalQuery.rules.length > 0);
