...
- **[Expérimental]** Spec 014 — Frontend hypermedia (HTMX + Tailwind v4) : Phases 1–7 livrées sur `feat/htmx-alpine-frontend`. Recherche SSR, facettes OOB, auth cookie, URL sync (hx-push-url + hx-history-elt), CSS buildé 22 Ko. Phase 7 : parité visuelle totale avec React — badges platform/type/year/access, titres (fix doc.titre), descriptions, group-hover, autocomplete HTMX (`/suggest`).
